...
namespace msgpack23
{
    ...
    [[nodiscard]] constexpr auto byteswap(T arg) noexcept
    {
        ...
        return std::byteswap(arg)
#else
        auto value_representation = std::bit_cast<std::array<std::byte, sizeof(T)>>(arg);
    ...
    private:
        ...
        {
            *store_++ = static_cast<B>(to_underlying(value));
        }
    ...
    private:
        ...
        {
            return current() == static_cast<std::byte>(to_underlying(value));
        }
    ...
    template <container Container, packable_object<Packer<
        typename Container::value_type, std::back_insert_iterator<Container>>> PackableObject>
    void pack(std::back_insert_iterator<Container> iterator, PackableObject const& obj)
    {
        return pack<typename Container::value_type, std::back_insert_iterator<Container>,
                    PackableObject>(iterator, obj);
    }
    ...
    template <typename UnpackableObject, span_convertible Container>
        requires unpackable_object<UnpackableObject, Unpacker<std::remove_const_t<std::ranges::range_value_t<
                                       Container>>>>
