...
distributed::dealer_socket::dealer_socket(zmq::context_t& ctx) : client_socket(ctx, zmq::socket_type::dealer)
{
}

void distributed::dealer_socket::send(const MsgType type, const std::vector<std::byte>& serialized)
{
...
std::tuple<MsgType, std::vector<std::byte>> distributed::dealer_socket::receive()
{
...
void distributed::dealer_socket::set_routing_id(const std::string& routingId)
{
    _socket.set(zmq::sockopt::routing_id, routingId);
}
